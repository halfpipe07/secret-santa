...
const CategorySelector = () => {
  ...
  const handlePressStart = (categoryId) => {
    if (hasSelected || categories.find(c => c.id === categoryId).selected) return;
    ...
    const interval = setInterval(() => {
      setProgress(prev => {
        ...
        return prev + 2; // Increment by 2 every 40ms for smooth animation over 2 seconds
      });
    }, 40);
      ...
      <div className="max-w-md mx-auto p-4">
        {categories.map((category) => {
          ...
          return (
            <div
              key={category.id}
              onPointerDown={() => handlePressStart(category.id)}
